...
package body Mozzoni.Client is
   ...
   -- Parse the available bytes in the Client's buffer
   procedure Parse_Available (Client : in out Client_Type);

   procedure Read_Available (Client : in out Client_Type) is
      Bytes_Read   : Integer := 0;
      Buffer       : aliased String (1 .. Read_Buffer_Size);
   begin

      loop
         Bytes_Read := Integer (Read_Socket (Client.Socket, Buffer'Address, Read_Buffer_Size));
   ...
   procedure Flush_All_Clients is
   begin
      Maps.Clear (Directory);
   end Flush_All_Clients;
